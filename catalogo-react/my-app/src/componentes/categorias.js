...
import { PaginacionCmd as Paginacion } from "../biblioteca/comunes";
...
export default class CategoriasMnt extends Component {
  ...
  list(num) {
    ...
    fetch(`${this.url}?sort=name&page=${pagina}&size=5`)
      .then((response) => {
        response.json().then(
          response.ok
            ? (data) => {
    ...
    if (this.form) {
      ...
      for (var cntr of this.form.elements) {
        if (cntr.categoria) {
          ...
          errors[cntr.categoria] = cntr.validationMessage;
